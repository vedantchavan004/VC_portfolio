...
skillCards.forEach(card => {
  ...
  const collapse = (c, panel) => {
    // set start height
    panel.style.maxHeight = panel.scrollHeight + 'px';
    // force reflow so the browser picks this as the start value
  ...
  header.addEventListener('click', () => {
    ...
    // close others with proper animation
    skillCards.forEach(other => {
      if (other !== card && other.classList.contains('expanded')) {
        collapse(other, other.querySelector('.card-content'));
      }
    });
  ...
  // keep height in sync when expanded and content wraps/resizes
