...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // --- Skills Accordion (robust height; no clipping) ---
  ...
  skillCards.forEach(card => {
    ...
    // Initialize heights: expanded panels get measured; others set to 0
    const initExpanded = card.classList.contains('expanded') || card.classList.contains('open');
    if (initExpanded) {
      // Ensure padding is active before measuring
      card.classList.add('expanded');
      // Force reflow then set maxHeight to full scroll height (includes padding)
    ...
    header.addEventListener('click', () => {
      ...
      if (isExpanded) {
        ...
        // Expand — add class first so padding applies, then measure
        card.classList.add('expanded');
        requestAnimationFrame(() => {
          content.style.maxHeight = content.scrollHeight + 'px';
        });
    ...
    // Keep height in sync when content reflows (responsive wraps, fonts, etc.)
    const ro = new ResizeObserver(() => {
      if (card.classList.contains('expanded')) {
        // Temporarily remove maxHeight to recalc natural size, then set
...
// ===== Skills accordion (scoped to #skills) =====
(function () {
  ...
  cards.forEach(card => {
    ...
    header.addEventListener('keydown', (e) => {
      if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); toggle(!card.classList.contains('open')); }
  ...
  // keep heights correct on resize
  window.addEventListener('resize', () => {
    document.querySelectorAll('#skills .skill-card.open .card-content')
      .forEach(c => c.style.maxHeight = c.scrollHeight + 'px');
  });
})();
